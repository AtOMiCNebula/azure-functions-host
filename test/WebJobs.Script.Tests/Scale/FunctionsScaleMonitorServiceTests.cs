...
namespace Microsoft.Azure.WebJobs.Script.Tests.Scale
{
    public class FunctionsScaleMonitorServiceTests
    {
        ...
        public FunctionsScaleMonitorServiceTests()
        {
            ...
            var options = new OptionsWrapper<ScaleOptions>(new ScaleOptions { ScaleMetricsSampleInterval = TimeSpan.FromMilliseconds(50) });
        ...
        public async Task OnTimer_PersistsMetrics()
        {
            ...
            Assert.Equal("Scale metrics sample for monitor 'testscalemonitor1': {\"Count\":10,\"TimeStamp\":\"0001-01-01T00:00:00\"}", logs[2].FormattedMessage);
        ...
        public async Task OnTimer_MonitorFailuresAreHandled()
        {
            ...
            Assert.Equal(5, logs.Count(p => p.FormattedMessage.Equals($"Failed to collect scale metrics sample for monitor 'testscalemonitor1'.")));

            // verify each successful sample is logged
            Assert.Equal(5, logs.Count(p => p.FormattedMessage.StartsWith($"Scale metrics sample for monitor 'testscalemonitor2'")));
    ...
    public class TestMetricsRepository : IScaleMetricsRepository
    {
        ...
        public Task<IDictionary<IScaleMonitor, IList<ScaleMetrics>>> ReadAsync(IEnumerable<IScaleMonitor> monitors)
        {
            return Task.FromResult<IDictionary<IScaleMonitor, IList<ScaleMetrics>>>(Metrics);
        }

        public Task WriteAsync(IDictionary<IScaleMonitor, ScaleMetrics> monitorMetrics)
        {
