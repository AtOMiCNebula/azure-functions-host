...
namespace Microsoft.Azure.WebJobs.Script.Tests.Scale
{
    public class FunctionsScaleManagerTests
    {
        ...
        public async Task GetScaleStatus_ReturnsExpectedResult()
        {
            ...
            mockMonitor1.Setup(p => p.GetScaleStatus(context)).Returns(new ScaleStatus { Vote = ScaleVote.ScaleIn });
            mockMonitor1.SetupGet(p => p.Id).Returns("testscalemonitor1");
            var mockMonitor2 = new Mock<IScaleMonitor>(MockBehavior.Strict);
            mockMonitor2.Setup(p => p.GetScaleStatus(context)).Returns(new ScaleStatus { Vote = ScaleVote.ScaleOut });
            mockMonitor2.SetupGet(p => p.Id).Returns("testscalemonitor2");
            var mockMonitor3 = new Mock<IScaleMonitor>(MockBehavior.Strict);
            mockMonitor3.Setup(p => p.GetScaleStatus(context)).Returns(new ScaleStatus { Vote = ScaleVote.ScaleIn });
            mockMonitor3.SetupGet(p => p.Id).Returns("testscalemonitor3");
            ...
            _metricsRepositoryMock.Setup(p => p.ReadAsync(It.IsAny<IEnumerable<IScaleMonitor>>())).ReturnsAsync(monitorMetrics);
        ...
        public async Task GetScaleStatus_MonitorFails_ReturnsExpectedResult()
        {
            ...
            mockMonitor2.Setup(p => p.GetScaleStatus(context)).Throws(exception);
            var mockMonitor3 = new Mock<IScaleMonitor>(MockBehavior.Strict);
            mockMonitor3.Setup(p => p.GetScaleStatus(context)).Returns(new ScaleStatus { Vote = ScaleVote.ScaleIn });
            mockMonitor3.SetupGet(p => p.Id).Returns("testscalemonitor3");
            List<IScaleMonitor> monitors = new List<IScaleMonitor>
        ...
        {
            ScaleStatusContext context = new ScaleStatusContext
            {
                WorkerCount = workerCount
            };
