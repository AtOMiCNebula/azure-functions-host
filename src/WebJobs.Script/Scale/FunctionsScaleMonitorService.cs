        ...
        private async void OnTimer(object state)
        {
            try
            {
                if (_primaryHostStateProvider.IsPrimary)
                {
                    ...
                    if (monitors.Any())
                    {
                        _logger.LogInformation($"Taking metrics samples for {monitors.Count()} monitor(s).");
                        ...
                        foreach (var monitor in monitors)
                        {
                            ...
                            try
                            {
                                ...
                                _logger.LogInformation($"Scale metrics sample for monitor '{monitor.Id}': {json}");
                            }
                            catch (Exception exc) when (!exc.IsFatal())
                            {
                                // if a particular monitor fails, log and continue
                                _logger.LogError(exc, $"Failed to collect scale metrics sample for monitor '{monitor.Id}'.");
                            }
                        }
